...
export function detectWorktree(cwd: string): WorktreeInfo {
  ...
  try {
    ...
    if (error instanceof Error && (error as NodeJS.ErrnoException).code !== 'ENOENT') {
      logger.warn('worktree', 'Unexpected error checking .git', { error: String(error) });
    }
  ...
  try {
    ...
    logger.warn('worktree', 'Failed to read .git file', { error: error instanceof Error ? error.message : String(error) });
    return NOT_A_WORKTREE;
  }
