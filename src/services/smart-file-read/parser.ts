  ...
  for (const [language, entry] of Object.entries(grammarsRaw as Record<string, unknown>)) {
    ...
    if (typeof queryPath === "string") {
      ...
      try {
        ...
        logger.warn('smart-file-read', `Custom query file not found: ${fullQueryPath}, falling back to generic`, {});
...
export function resolveGrammarPathWithFallback(language: string, projectRoot?: string): string | null {
  ...
  logger.warn('smart-file-read', `Grammar package not found for "${language}": ${entry.package} (install it in your project's node_modules)`, {});
  return null;
}
