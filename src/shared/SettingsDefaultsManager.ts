...
export class SettingsDefaultsManager {
  private static readonly DEFAULTS: SettingsDefaults = {
    ...
    CLAUDE_MEM_SERVER_URL: `http://127.0.0.1:${process.env.CLAUDE_MEM_SERVER_PORT ?? String(37877 + ((process.getuid?.() ?? 77) % 100))}`,  // Default server-beta runtime URL — UID-derived for multi-account isolation
    CLAUDE_MEM_SERVER_API_KEY: '',                     // Local hook API key, populated by installer when runtime=server-beta
    CLAUDE_MEM_SERVER_PROJECT_ID: '',                  // Default Postgres project_id used by hooks when runtime=server-beta
  };
