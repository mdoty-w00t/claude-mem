...
describe("Observation I/O event handlers", () => {
  ...
  it("before_agent_start calls init on worker", async () => {
  ...
  it("after_compaction logs session preservation (no re-init)", async () => {
    const { api, logs, fireEvent } = createMockApi({ workerPort });
    claudeMemPlugin(api);
    ...
    const initRequests = receivedRequests.filter((r) => r.url === "/api/sessions/init");
    assert.equal(initRequests.length, 0, "after_compaction should not send init");
    assert.ok(logs.some((l) => l.includes("preserved after compaction")), "should log compaction");
  });
